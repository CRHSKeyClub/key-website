#!/usr/bin/env python3
"""
Auto-fix ALL t-shirt size discrepancies between students_rows.csv and Google Form responses.
This script automatically fixes all discrepancies without user interaction.
"""
...
def fix_all_discrepancies(discrepancies):
    ...
    backup_file = Path('/Users/parthzanwar/Downloads/students_rows_auto_backup.csv')
...
def generate_sql_statements(discrepancies):
    ...
    print("="*60)
    print("-- Auto-fixed T-Shirt Size Discrepancies")
    print("-- Generated on", __import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    print(f"-- Total records updated: {len(discrepancies)}")
    print()
...
def main():
    """Main function to auto-fix all t-shirt size discrepancies."""
    print("🔧 AUTO-FIX ALL T-SHIRT SIZE DISCREPANCIES")
    print("="*60)
    ...
    if success:
        ...
        generate_sql_statements(discrepancies)
        ...
        if all_correct:
            ...
            print(f"✅ Auto-fixed {len(discrepancies)} t-shirt size discrepancies")
        ...
        print(f"📁 Original file backed up to: students_rows_auto_backup.csv")
        
        # Show what was fixed
        print("\nFIXED DISCREPANCIES:")
        print("-" * 40)
        for disc in discrepancies:
            print(f"• {disc['s_number']} ({disc['name']}): {disc['database_size']} → {disc['form_size']}")
    else:
